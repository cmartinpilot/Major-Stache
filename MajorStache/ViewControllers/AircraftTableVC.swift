...
class AircraftTableVC: UITableViewController, CoreDataConforming {
    ...
    fileprivate lazy var fetchedResultsController:NSFetchedResultsController<Aircraft>? = {
        ...
        let controller:NSFetchedResultsController<Aircraft>? = (self.dataManager?.fetchedResultsController(sortDescriptors: [sortDescriptors], predicate: nil, delegate: self))
